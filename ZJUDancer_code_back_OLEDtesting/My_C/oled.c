...
void OLED_ShowChar_my2(uint8_t x,uint8_t y,uint8_t chr,uint8_t Char_Size)
{      	
	unsigned char c=0,i=0;	
	uint8_t x1, y1;
	...
	//x1 = 128 - (x+8);
	x1 = x + 8;//x1 = x + n, n ±íÊ¾Ïò×óµÄÆ«ÒÆÁ¿
	y1 = y;
	
	OLED_Set_Pos(x1,y1);	
	for(i=0;i<8;i++)
		OLED_WR_Byte(F8X16[c*16+(7-i)],OLED_DATA);
	OLED_Set_Pos(x1,y1+1);
	for(i=0;i<8;i++)
		OLED_WR_Byte(F8X16[c*16+(7-i)+8],OLED_DATA);

// 	for(i=0;i<8;i++)
// 		OLED_WR_Byte(F8X16[c*16+i],OLED_DATA);
// 	OLED_Set_Pos(x1,y1+1);
// 	for(i=0;i<8;i++)
// 		OLED_WR_Byte(F8X16[c*16+i+8],OLED_DATA);//»áµ¼ÖÂ×Ö·û×óÓÒµßµ¹

}


void OLED_ShowChar_my(uint8_t x,uint8_t y,uint8_t chr,uint8_t Char_Size)
{      	
	unsigned char c=0,i=0;	
	uint8_t x1, y1;
	c=chr-' ';//µÃµ½Æ«ÒÆºóµÄÖµ			
...
{ 	
 	...
 	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOB|RCC_AHB1Periph_GPIOB, ENABLE);	 //Ê¹ÄÜPC,D,G¶Ë¿ÚÊ±ÖÓ

	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_13|GPIO_Pin_15;	 //PD3,PD6ÍÆÍìÊä³ö  
 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT; 		 //ÍÆÍìÊä³ö
	GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;//ËÙ¶È50MHz
 	GPIO_Init(GPIOB, &GPIO_InitStructure);	  //³õÊ¼»¯GPIOD3,6
 	GPIO_SetBits(GPIOB, GPIO_Pin_13|GPIO_Pin_15);	//PD3,PD6 Êä³ö¸ß
/*
 #if OLED_MODE==1
...
void OLED_Show_Char_Line(uint8_t y, int8_t * str, uint8_t num)
{
	uint8_t i = 0;
	
	for(i = 0; i < num; i++)
		OLED_ShowChar_my2(8*i, y*2, str[i], 16);
}


int printf_oled(uint8_t line, const char *fmt, ...)	
{
	char s[100];
	va_list argptr;
	int cnt;
	...
	va_end(argptr);
	OLED_Show_Char_Line(line, (int8_t *)s, cnt);
	return(cnt);
}
